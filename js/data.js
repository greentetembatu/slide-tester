/*<!--==================sixBuildStBusinessPackages ==================-->*/

const sixBuildStBusinessPackagesData = {
  section: {
    id: "sixBuildStBusinessPackages",
    title: "Bangun Bisnis Anda",
    subtitle: "Pilih paket pendirian perusahaan yang sesuai dengan kebutuhan bisnis Anda"
  },
  packages: [
    {
      badge: "POPULAR",
      type: "Paket Pendirian",
      name: "PT. Lengkap",
      description: "Paket lengkap pendirian PT. Proses cepat dan terpercaya sesuai degan regulasi.",
      features: [
        "Pengecekan Nama PT.",
        "Akta Pendirian Notaris",
        "SK Kemenkumham"
      ],
      detailLink: "pembuatan-pt.html",
      price: "Rp 4.000.000",
      contactLink: "https://wa.me/6281932122224"
    },
    {
      badge: "POPULAR",
      type: "Paket Pendirian",
      name: "CV. Lengkap",
      description: "Paket lengkap pendirian CV. Proses cepat dan terpercaya sesuai degan regulasi.",
      features: [
        "Pengecekan Nama CV.",
        "Akta Pendirian Notaris",
        "SK Kemenkumham"
      ],
      detailLink: "pembuatan-cv.html",
      price: "Rp 3.000.000",
      contactLink: "https://wa.me/6281932122224"
    },
    {
      badge: "POPULAR",
      type: "Paket Pendirian",
      name: "KOPERASI",
      description: "Paket lengkap pendirian KOPERASI Proses cepat dan terpercaya sesuai degan regulasi.",
      features: [
        "Pengecekan Nama Koperasi",
        "Akta Pendirian Notaris",
        "SK Kemenkumham"
      ],
      detailLink: "pembuatan-koperasi.html",
      price: "Rp 3.000.000",
      contactLink: "https://wa.me/6281932122224"
    },
    {
      badge: "POPULAR",
      type: "Paket Pendirian",
      name: "YAYASAN",
      description: "Paket lengkap pendirian YAYASAN Proses cepat dan terpercaya sesuai degan regulasi.",
      features: [
        "Pengecekan Nama Yayasan",
        "Akta Pendirian Notaris",
        "SK Kemenkumham"
      ],
      detailLink: "pembuatan-yayasan.html",
      price: "Rp 3.500.000",
      contactLink: "https://wa.me/6281932122224"
    },
    {
      badge: "POPULAR",
      type: "Paket Pendirian",
      name: "FIRMA",
      description: "Paket lengkap pendirian FIRMA Proses cepat dan terpercaya sesuai degan regulasi.",
      features: [
        "Pengecekan Nama Firma.",
        "Akta Pendirian Notaris",
        "SK Kemenkumham"
      ],
      detailLink: "pembuatan-firma.html",
      price: "Rp 3.000.000",
      contactLink: "https://wa.me/6281932122224"
    },
    {
      badge: "POPULAR",
      type: "Paket Pendirian",
      name: "PMA",
      description: "Paket lengkap pendirian PMA Proses cepat dan terpercaya sesuai degan regulasi.",
      features: [
        "Pengecekan Nama PMA.",
        "Akta Pendirian Notaris",
        "SK Kemenkumham"
      ],
      detailLink: "pembuatan-pma.html",
      price: "Rp 3.000.000",
      contactLink: "https://wa.me/6281932122224"
    }
  ]
};



/*<!--==================businessPackagesCompletePt==================-->*/

const businessPackagesDataPt = {
  completePt: {
    sectionId: "businessPackagesCompletePt",
    title: "Bangun Bisnis Anda",
    subtitle: "Pilih paket pendirian perusahaan yang sesuai dengan kebutuhan bisnis Anda",
    type: "Paket Pendirian",
    name: "PT. Lengkap",
    description: "Paket lengkap pendirian PT. Proses cepat dan terpercaya sesuai dengan regulasi.",
    features: [
        "Pengecekan Nama PT.",
        "Akta Pendirian Notaris",
        "SK Kemenkumham",
        "NPWP Badan",
        "SKT KPP",
        "NIB",
        "Akun OSS RBA",
        "Rekening Giro"
    ],
    bonus: [
      "Rekening Perusahaan"
    ],
    price: "Rp 4.000.000",
    contactLink: "https://wa.me/6281932122224",
    image: {
      src: "img/women-office.png",
      alt: "Office Professional"
    }
  }
};













/*<!--==================businessPackagesCompleteCv==================-->*/

const businessPackagesDataCv = {
  completeCv: {
    sectionId: "businessPackagesCompleteCv",
    title: "Bangun Bisnis Anda",
    subtitle: "Pilih paket pendirian perusahaan yang sesuai dengan kebutuhan bisnis Anda",
    type: "Paket Pendirian",
    name: "CV. Lengkap",
    description: "Paket lengkap pendirian CV. Proses cepat dan terpercaya sesuai dengan regulasi.",
    features: [
        "Pengecekan Nama CV.",
        "Akta Pendirian Notaris",
        "SK Kemenkumham",
        "NPWP Badan",
        "SKT KPP",
        "NIB",
        "Akun OSS RBA",
        "Rekening Giro"
    ],
    bonus: [
      "-",
      "-"
    ],
    price: "Rp 3.000.000",
    contactLink: "https://wa.me/6281932122224",
    image: {
      src: "img/room4.jpg",
      alt: "Office Professional"
    }
  }
};







const businessPackagesDataKoperasi = {
  completeKoperasi: {
    sectionId: "businessPackagesCompleteKoperasi",
    title: "Bangun Bisnis Anda",
    subtitle: "Pilih paket pendirian perusahaan yang sesuai dengan kebutuhan bisnis Anda",
    type: "Paket Pendirian",
    name: "KOPERASI",
    description: "Paket lengkap pendirian Koperasi. Proses cepat dan terpercaya sesuai dengan regulasi.",
    features: [
        "Pengecekan Nama Koperasi.",
        "Akta Pendirian Notaris",
        "SK Kemenkumham",
        "NPWP Badan",
        "SKT KPP",
        "NIB",
        "Akun OSS RBA",
        "Rekening Giro"
    ],
    bonus: [
      "-",
      "-"
    ],
    price: "Rp 3.000.000",
    contactLink: "https://wa.me/6281932122224",
    image: {
      src: "img/room-3.jpg",
      alt: "Office Professional"
    }
  }
};









const businessPackagesDataYayasan = {
  completeYayasan: {
    sectionId: "businessPackagesCompleteYayasan",
    title: "Bangun Bisnis Anda",
    subtitle: "Pilih paket pendirian perusahaan yang sesuai dengan kebutuhan bisnis Anda",
    type: "Paket Pendirian",
    name: "YAYASAN",
    description: "Paket lengkap pendirian Yayasan. Proses cepat dan terpercaya sesuai dengan regulasi.",
    features: [
        "Pengecekan Nama Yayasan.",
        "Akta Pendirian Notaris",
        "SK Kemenkumham",
        "NPWP Badan",
        "SKT KPP",
        "NIB",
        "Akun OSS RBA",
        "Rekening Giro"
    ],
    bonus: [
      "-",
      "-"
    ],
    price: "Rp 3.500.000",
    contactLink: "https://wa.me/6281932122224",
    image: {
      src: "img/room-2.jpg",
      alt: "Office Professional"
    }
  }
};







const businessPackagesDataFirma = {
  completeFirma: {
    sectionId: "businessPackagesCompleteFirma",
    title: "Bangun Bisnis Anda",
    subtitle: "Pilih paket pendirian perusahaan yang sesuai dengan kebutuhan bisnis Anda",
    type: "Paket Pendirian",
    name: "FIRMA",
    description: "Paket lengkap pendirian Firma. Proses cepat dan terpercaya sesuai dengan regulasi.",
    features: [
        "Pengecekan Nama Firma.",
        "Akta Pendirian Notaris",
        "SK Kemenkumham",
        "NPWP Badan",
        "SKT KPP",
        "NIB",
        "Akun OSS RBA",
        "Rekening Giro"
    ],
    bonus: [
      "-",
      "-"
    ],
    price: "Rp 3.000.000",
    contactLink: "https://wa.me/6281932122224",
    image: {
      src: "img/room-1.jpg",
      alt: "Office Professional"
    }
  }
};








const businessPackagesDataPma = {
  completePma: {
    sectionId: "businessPackagesCompletePma",
    title: "Bangun Bisnis Anda",
    subtitle: "Pilih paket pendirian perusahaan yang sesuai dengan kebutuhan bisnis Anda",
    type: "Paket Pendirian",
    name: "PMA",
    description: "Paket lengkap pendirian PMA. Proses cepat dan terpercaya sesuai dengan regulasi.",
    features: [
        "Pengecekan Nama PMA.",
        "Akta Pendirian Notaris",
        "SK Kemenkumham",
        "NPWP Badan",
        "SKT KPP",
        "NIB",
        "Akun OSS RBA",
        "Rekening Giro"
    ],
    bonus: [
      "-",
      "-"
    ],
    price: "Rp 3.000.000",
    contactLink: "https://wa.me/6281932122224",
    image: {
      src: "img/room4.jpg",
      alt: "Office Professional"
    }
  }
};



/*<!--==================view-about-explan==================-->*/
const aboutExplanationSections = {
  notarisWenika: {
    sectionId: "viewAboutExplan",
    title: "MASTER LEGAL",
    subtitle: "Solusi kebutuhan bisnis dan legalitas anda",
    description:
      "Master Legal adalah perusahaan jasa yang bergerak di bidang layanan notaris, khususnya untuk pendirian berbagai bentuk badan usaha dan Perusahaan. Dengan tim notaris dan konsultan hukum yang berpengalaman, kami memberikan solusi lengkap mulai dari konsultasi awal hingga akhir. Tujuan kami adalah memudahkan klien dalam mewujudkan entitas bisnis yang legal dan kuat secara hukum.",
    serviceTitle: "Ruang Lingkup Pelayanan:",
    services: [
      {
        title: "Pendirian PT",
        desc: "Asistensi lengkap mulai dari pengecekan nama, SK Kemenkumham, NPWP Badan, pengurusan NIB. dan yang lainnya."
      },
      {
        title: "Pendirian CV",
        desc: "Solusi tepat bagi usaha dengan karakteristik mitra aktif dan pasif."
      },
      {
        title: "Pembentukan Yayasan",
        desc: "Mewujudkan visi sosial melalui pendirian perkumpulan berbadan hukum."
      },
      {
        title: "Pendirian Firma (Fa)",
        desc: "Melayani persekutuan untuk profesional dengan nama bersama."
      },
      {
        title: "Pembentukan Koperasi",
        desc: "Mendukung ekonomi kerakyatan sesuai prinsip koperasi."
      },
      {
        title: "PMA",
        desc: "Mengakomodasi kebutuhan secara legal."
      }
    ],
    button: {
      text: "Konsultasi Sekarang",
      link: "https://wa.me/6281932122224"
    }
  }

};


/*<!--==================tree-superior-services==================-->*/
const superiorServicesSections = {
  notaryMainServices: {
    sectionId: "treeSuperiorServices",
    title: "LAYANAN UNGGULAN",
    subtitle: "Kami menyediakan berbagai layanan notaris untuk kebutuhan hukum Bisnis Anda",
    services: [
      {
        icon: "fas fa-file-contract",
        title: "Pembuatan Akta",
        description:
          "Pembuatan akta notaris untuk berbagai keperluan hukum seperti perjanjian, wasiat, dan pernyataan."
      },
      {
        icon: "fas fa-building",
        title: "Pendirian Perusahaan",
        description:
          "Pendirian badan usaha seperti PT, CV, Firma, Koperasi, Yayasan, dan PMA."
      },
      {
        icon: "fas fa-gavel",
        title: "Konsultasi Hukum",
        description:
          "Konsultasi hukum untuk perorangan dan perusahaan terkait dengan dokumen dan transaksi hukum."
      }
    ]
  }

};


/*<!--==================WHY US SECTION ==================-->*/
const whyUsData = {
  sectionClass: "why-us-section",

  header: {
    class: "why-us-header",
    title: "Why Choose Us",
    underlineClass: "underline",
  },

  gridClass: "why-us-grid",

  items: [
    {
      iconClass: "fa-solid fa-gauge-high",
      title: "Proses Cepat",
      description:
        "Dengan tim berpengalaman, kami menawarkan layanan yang cepat, responsif, dan memberikan update secara berkala.",
    },
    {
      iconClass: "fa-solid fa-tags",
      title: "Harga Kompetitif",
      description:
        "Kami berkomitmen untuk menawarkan harga yang kompetitif dibandingkan dengan pasar.",
    },
    {
      iconClass: "fa-solid fa-handshake-angle",
      title: "Ramah dan Profesional",
      description:
        "Kami menyediakan layanan customer service yang ramah dan profesional, berusaha semaksimal mungkin mempermudah proses bagi seluruh pelanggan kami.",
    },
    {
      iconClass: "fas fa-skiing",
      title: "Praktis",
      description:
        "Layanan kami mencakup seluruh Indonesia untuk pendirian usaha, dan seluruh proses dapat dilakukan secara online.",
    },
    {
      iconClass: "fas fa-balance-scale",
      title: "Terpercaya",
      description:
        "Tim kami terdiri dari para profesional berpengalaman. Seluruh proses legalitas ditangani langsung oleh Tim kami tanpa pihak ketiga.",
    },
    {
      iconClass: "fas fa-percentage",
      title: "Garansi 100%",
      description:
        "Kami sangat memahami betapa pentingnya kepercayaan pelanggan. Oleh karena itu, kami menawarkan garansi 100% untuk setiap layanan.",
    },
  ],
};

/*<!--==================VIEW ND SECTION ==================-->*/

const viewNdInfoData = {
  sectionClass: "view-nd-info-section",
  particlesClass: "view-nd-info-particles",
  containerClass: "view-nd-info-container",
  columnClass: "view-nd-info-column",
  listClass: "view-nd-info-steps-list",

  columns: [
    {
      title: "PERSYARATAN",
      items: [
        "Kartu Tanda Penduduk (KTP)",
        "Kartu Keluarga (KK)",
        "NPWP Pribadi",
        "Tujuan kegiatan usaha harus jelas",
      ],
    },
    {
      title: "ALUR PEMESANAN",
      items: [
        "Menghubungi Call Center Kami",
        "Estimasi penerbitan maksimal 2 hari kerja setelah penandatanganan Minuta.",
      ],
    },
  ],
};

/*<!--==================PARALLAX SECTION ==================-->*/

const parallaxSectionData = {
  sectionClass: "parallax-section",
  containerClass: "container-cards",

  cards: [
    {
      iconClass: "fa-solid fa-check-double",
      title: "Mutu Sesuai Standar",
      description:
        "Pelaksanaan pekerjaan dengan perencanaan teliti, direview secara bertahap dari tim pelaksana hingga partner, untuk mengantisipasi kesalahan dan terus meningkatkan kualitas.",
    },
    {
      iconClass: "fa-solid fa-thumbs-up",
      title: "Hasil Terbaik",
      description:
        "Kami memiliki misi untuk memberikan hasil pekerjaan yang terbaik kepada Klien. Untuk itu, Konsulpedia telah menerapkan standar kualitas yang tinggi akan Pekerjaan.",
    },
    {
      iconClass: "fa-solid fa-dollar",
      title: "Biaya Terjangkau",
      description:
        "Walaupun kami memberikan kualitas terbaik kepada Klien. Kami tetap menawarkan solusi yang terjangkau untuk memenuhi kebutuhan bisnis Anda dan menawarkan biaya yang terjangkau.",
    },
  ],
};

/*======ISO SECTION===========*/

const ISO_DATA = {
  title: "LEGALITAS",
  description:
    "Kami telah memenuhi standar dalam pengelolaan layanan dan operasional pengurusan Izin , sebagai bentuk komitmen terhadap kualitas dan profesionalisme.",
  certificates: [
    {
      image: "img/dokumen izin dokumen lainnya.png",
      alt: "dokumen izin 1",
    },
    /*{
      image: "img/dokumen izin praktek.png",
      alt: "dokumen izin 2",
    },*/
    {
      image: "img/dokumen izin SK Kemenkumham.png",
      alt: "dokumen izin 3",
    },
  ],
};

/*======TESTIMONI SECTION===========*/

const TESTIMONIAL_DATA = {
  title: "Apa Kata Klien Kami",
  subtitle: "Kepercayaan klien adalah prioritas utama kami",
  testimonials: [
    {
      clientPhoto: "img/photo-team/my-team-1 (1).jpg",
      clientName: "Andi Pratama",
      clientRole: "Direktur • PT Maju Bersama",
      text: "Pelayanan sangat profesional dan cepat. Tim sangat responsif dan membantu dari awal hingga selesai.",
      date: "12 Juni 2025",
      socials: [
        { icon: "fab fa-linkedin", url: "#" },
        { icon: "fab fa-instagram", url: "#" },
      ],
    },
    {
      clientPhoto: "img/photo-team/my-team-1 (2).jpg",
      clientName: "Siti Rahma",
      clientRole: "HR Manager • CV Sukses Mandiri",
      text: "Sangat direkomendasikan. Proses legalitas cepat dan jelas, tanpa ribet.",
      date: "28 Mei 2025",
      socials: [{ icon: "fab fa-instagram", url: "#" }],
    },
    {
      clientPhoto: "img/photo-team/my-team-1 (3).jpg",
      clientName: "Budi Santoso",
      clientRole: "Owner • PT Nusantara Jaya",
      text: "Pendampingan sangat detail dan transparan. Sangat membantu bisnis kami berkembang.",
      date: "3 Mei 2025",
      socials: [{ icon: "fab fa-linkedin", url: "#" }],
    },
  ],
};

/*<!--===============view-career-root===============-->*/
const viewCareerJobVacancySectionData = {
  sectionClass: "view-career-section",
  containerClass: "view-career-container",

  heading: {
    tag: "h2",
    text: "MULAI KARIRMU DI SINI",
  },

  gridClass: "view-career-grid",

  cards: [
    {
      status: "Tersedia",
      position: "Accounting",
      qualificationLabel: "Kualifikasi:",
      qualificationText: "Wanita, Pendidikan S1., Bersedia ditempatkan di seluruh cabang kami.",
      applyLink: "mailto:wenikapriastuti@gmail.com",
      applyBtnClass: "view-btn-apply-carrer",
      applyText: "Lamar Sekarang",
    },
    {
      status: "Tersedia",
      position: "Marketing",
      qualificationLabel: "Kualifikasi:",
      qualificationText: "Wanita, Pendidikan SMA-SMK., Bersedia ditempatkan di seluruh cabang kami.",
      applyLink: "mailto:wenikapriastuti@gmail.com",
      applyBtnClass: "view-btn-apply-carrer",
      applyText: "Lamar Sekarang",
    },
    {
      status: "Tersedia",
      position: "Digital Marketing",
      qualificationLabel: "Kualifikasi:",
      qualificationText: "Wanita, Pendidikan SMA/SMK, Mahir Menggunakan Gadjet/ Perangkat IT dan Sosial Media., Bersedia ditempatkan di seluruh cabang kami.",
      applyLink: "mailto:wenikapriastuti@gmail.com",
      applyBtnClass: "view-btn-apply-carrer",
      applyText: "Lamar Sekarang",
    },
    {
      status: "Tersedia",
      position: "Legal Officer",
      qualificationLabel: "Kualifikasi:",
      qualificationText: "Wanita, Pendidikan S1., Bersedia ditempatkan di seluruh cabang kami.",
      applyLink: "mailto:wenikapriastuti@gmail.com",
      applyBtnClass: "view-btn-apply-carrer",
      applyText: "Lamar Sekarang",
    },
    {
      status: "Tersedia",
      position: "Admin Support",
      qualificationLabel: "Kualifikasi:",
      qualificationText: "Wanita, Pendidikan S1., Bersedia ditempatkan di seluruh cabang kami.",
      applyLink: "mailto:wenikapriastuti@gmail.com",
      applyBtnClass: "view-btn-apply-carrer",
      applyText: "Lamar Sekarang",
    },
  ],
};

/*<!--===============view-target-vision-mission-root===============-->*/
const viewTargetVisionMissionSectionData = {
  sectionClass: "view-target-content",
  containerClass: "view-target-container",

  titleSection: {
    wrapperClass: "view-target-section-title",
    title: "VISI & MISI KAMI",
    dividerClass: "view-target-divider",
  },

  visionMissionWrapperClass: "view-target-vision-mission",

  vision: {
    boxClass: "view-target-vision-box",
    titleHTML: '<i class="fas fa-eye"></i> VISI kami',
    description:
      "Menjadi mitra terpercaya dan solusi terdepan dalam mengoptimalkan kinerja bisnis klien melalui layanan konsultan yang inovatif dan berkualitas tinggi.",
  },

  mission: {
    boxClass: "view-target-mission-box",
    titleHTML: '<i class="fas fa-bullseye"></i> MISI Kami',
    description:
      "Memberikan solusi konsultan profesional dan terpercaya untuk kepuasan klien dan pertumbuhan bisnis yang berkelanjutan.",
  },
};

/*<!--===============ARTIKEL===============-->*/

const articleData = [
  {
    id: 1,
    title: "Strategi Branding Usaha Pulsa: Rahasia Kuatkan Bisnis",
    image: "img/meeting-1.jpg",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit.\n\n Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit. \n\n Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit.\n\n Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit. ",
    author: "Wenika Priastuti",
    date: "December 24, 2025",
  },
  {
    id: 2,
    title: "KBLI 30300: Industri Strategis Sarat Regulasi",
    image: "img/meeting-3.jpg",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit.\n\n Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit. \n\n Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit.\n\n Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit. ",
    author: "Wenika Priastuti",
    date: "December 24, 2025",
  },
  {
    id: 3,
    title: "Pentingnya Memilih Izin Usaha yang Tepat (KBLI)",
    image: "img/room4.jpg",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit.\n\n Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit. \n\n Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit.\n\n Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde voluptate aperiam deserunt commodi dolores impedit. Quod deserunt distinctio odio dicta tempora amet autem. Nobis quaerat qui sint distinctio veritatis quis saepe nostrum officiis asperiores adipisci, eligendi quam nihil labore fuga non assumenda inventore reprehenderit tenetur similique accusamus doloribus, quidem sit. ",
    author: "Wenika Priastuti",
    date: "December 23, 2025",
  },
];

/*<!--===============TEAM SECTION===============-->*/
const tm_teamData = [
  {
    id: 1,
    name: "Wenika Priastuti, SH., M.Kn",
    role: "Chief Executive Officer",
    image:
      "img/wenika-priastuti.jpg",
    shortDesc:
      "Lorem ipsum dolor, sit amet consectetur adipisicing.",
    fullDesc:
      "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Qui, earum quae quos iste sed dicta quasi labore voluptatum natus exercitationem.. \n\nLorem ipsum dolor, sit amet consectetur adipisicing elit. Qui, earum quae quos iste sed dicta quasi labore voluptatum natus exercitationem.",
  },
  {
    id: 2,
    name: "Sarah Wijaya",
    role: "Creative Director",
    image:
      "img/photo-team/my-team-1 (1).jpg",
    shortDesc:
      "Lorem ipsum dolor, sit amet consectetur adipisicing.",
    fullDesc:
      "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Qui, earum quae quos iste sed dicta quasi labore voluptatum natus exercitationem.. \n\nLorem ipsum dolor, sit amet consectetur adipisicing elit. Qui, earum quae quos iste sed dicta quasi labore voluptatum natus exercitationem.",
  },
  {
    id: 3,
    name: "David Kurnia",
    role: "Notaris",
    image:
      "img/photo-team/my-team-1 (2).jpg",
    shortDesc:
      "Lorem ipsum dolor, sit amet consectetur adipisicing.",
    fullDesc:
      "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Qui, earum quae quos iste sed dicta quasi labore voluptatum natus exercitationem.. \n\nLorem ipsum dolor, sit amet consectetur adipisicing elit. Qui, earum quae quos iste sed dicta quasi labore voluptatum natus exercitationem.",
  },
  {
    id: 4,
    name: "Rina Kartika",
    role: "Marketing Manager",
    image:
      "img/photo-team/my-team-1 (3).jpg",
    shortDesc:
      "Lorem ipsum dolor, sit amet consectetur adipisicing.",
    fullDesc:
      "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Qui, earum quae quos iste sed dicta quasi labore voluptatum natus exercitationem.. \n\nLorem ipsum dolor, sit amet consectetur adipisicing elit. Qui, earum quae quos iste sed dicta quasi labore voluptatum natus exercitationem.",
  },
];

/*======CLIENT LOGO SECTION===========*/

const CLIENT_LOGO_DATA = {
  title: 'Telah Dipercaya Lebih dari <span id="clientCount"></span> Klien',
  logos: [
    { image: "img/brand-mitra/mitra-1 (1).png", alt: "Client 1" },
    { image: "img/brand-mitra/mitra-1 (2).png", alt: "Client 2" },
    { image: "img/brand-mitra/mitra-1 (3).png", alt: "Client 3" },
    { image: "img/brand-mitra/mitra-1 (4).png", alt: "Client 4" },
    { image: "img/brand-mitra/mitra-1 (5).png", alt: "Client 5" },
    { image: "img/brand-mitra/mitra-1 (6).png", alt: "Client 6" },
    { image: "img/brand-mitra/mitra-1 (7).png", alt: "Client 7" },
    { image: "img/brand-mitra/mitra-1 (8).png", alt: "Client 8" },
    { image: "img/brand-mitra/mitra-1 (9).png", alt: "Client 9" },
    { image: "img/brand-mitra/mitra-1 (10).png", alt: "Client 10" },
    { image: "img/brand-mitra/mitra-1 (11).png", alt: "Client 11" },
    { image: "img/brand-mitra/mitra-1 (1).png", alt: "Client 12" },
    { image: "img/brand-mitra/mitra-1 (2).png", alt: "Client 13" },
    { image: "img/brand-mitra/mitra-1 (3).png", alt: "Client 14" },
    { image: "img/brand-mitra/mitra-1 (4).png", alt: "Client 15" },
    { image: "img/brand-mitra/mitra-1 (5).png", alt: "Client 16" },
    { image: "img/brand-mitra/mitra-1 (6).png", alt: "Client 17" },
    { image: "img/brand-mitra/mitra-1 (7).png", alt: "Client 18" },
    { image: "img/brand-mitra/mitra-1 (8).png", alt: "Client 19" },
    { image: "img/brand-mitra/mitra-1 (9).png", alt: "Client 20" },
    { image: "img/brand-mitra/mitra-1 (10).png", alt: "Client 21" },
    { image: "img/brand-mitra/mitra-1 (11).png", alt: "Client 22" },
    // tinggal tambah sampai 40 tanpa sentuh HTML
  ],
};

/*=======TREE STEEP==========*/
const viewTabBusinessLicenseStepsSectionData = {
  sectionClass: "view-tab-steps-section",

  content: {
    wrapperClass: "view-tab-steps-content",

    heading: {
      tag: "h2",
      text: "3 LANGKAH MUDAH MENGURUS PERIZINAN USAHA",
    },

    subDescription: {
      class: "view-tab-sub-desc",
      text: `Kami memberikan kemudahan bagi Anda dalam perizinan dan pembuatan
      Perusahaan mendapatkan layanan yang dibutuhkan yaitu dengan langkah
      berikut:`,
    },

    steps: [
      {
        itemClass: "view-tab-step-item",
        icon: {
          src: "img/icon/3120612.png",
          alt: "Icon",
          class: "view-tab-step-icon",
        },
        text: {
          title: "Konsultasi",
          description:
            "Anda hanya perlu menghubungi Customer Service kami dan menjelaskan layanan yang sedang anda butuhkan.",
        },
      },
      {
        itemClass: "view-tab-step-item",
        icon: {
          src: "img/icon/2620815.png",
          alt: "Icon",
          class: "view-tab-step-icon",
        },
        text: {
          title: "Pengisian Data",
          description:
            "Anda akan didampingi dan dipandu oleh tim kami dalam proses pengisian data yang diperlukan.",
        },
      },
      {
        itemClass: "view-tab-step-item",
        icon: {
          src: "img/icon/1055666.png",
          alt: "Icon",
          class: "view-tab-step-icon",
        },
        text: {
          title: "Proses Pengerjaan",
          description:
            "Kami akan melakukan proses pengerjaan serta akan memberikan update progres pekerjaan sampai selesai.",
        },
      },
    ],
  },

  imageSide: {
    wrapperClass: "view-tab-steps-image",
    image: {
      src: "img/meeting-3.jpg",
      alt: "tim notaris wenika",
    },
  },
};









/*===========FAQ==================*/
/*===========FAQ==================*/
/*===========FAQ==================*/

const viewFaqBusinessEntitySectionData = {
  sectionClass: "view-faq-section",

  header: {
    title: "Pertanyaan yang Sering Diajukan",
    description:
      "Beberapa pertanyaan yang sering ditanyakan oleh pengusaha terkait badan usaha"
  },

  columns: [
    {
      items: [
        {
          question:
            "Apakah dapat membantu membuat PT diseluruh Indonesia?",
          answer:
            "Ya, Kami dapat melayani pengurusan legalitas di seluruh wilayah Indonesia secara digital dan terintegrasi."
        },
        {
          question:
            "Dokumen apa saja yang dibutuhkan untuk membuat PT / CV?",
          answer:
            "Dokumen yang dibutuhkan termasuk NPWP dan KTP Direktur, Komisaris, dan Pemegang Saham."
        },
        {
          question: "Apakah bisa mendirikan PT hanya 1 orang?",
          answer:
            "Ya, Anda bisa mendirikan PT Perorangan yang khusus diperuntukkan bagi usaha mikro dan kecil dengan 1 orang pendiri."
        },
        {
          question: "Apakah bisa mendirikan PT hanya 1 orang?",
          answer:
            "Ya, Anda bisa mendirikan PT Perorangan yang khusus diperuntukkan bagi usaha mikro dan kecil dengan 1 orang pendiri."
        }
      ]
    },
    {
      items: [
        {
          question: "Apa Perbedaan PT dan CV?",
          answer:
            "Perbedaan utama terletak pada status badan hukum, pemisahan harta kekayaan, dan struktur kepengurusan."
        },
        {
          question: "Berapa lama proses pembuatan badan usaha?",
          answer:
            "Estimasi pengerjaan biasanya memakan waktu 2-5 hari kerja setelah dokumen lengkap ditandatangani."
        },
        {
          question: "Bagaimana dengan sistem pembayarannya?",
          answer:
            "Sistem pembayaran dilakukan secara bertahap atau sesuai kesepakatan dengan jaminan keamanan transaksi."
        },
        {
          question: "Bagaimana dengan sistem pembayarannya?",
          answer:
            "Sistem pembayaran dilakukan secara bertahap atau sesuai kesepakatan dengan jaminan keamanan transaksi."
        }
      ]
    }
  ]
};
